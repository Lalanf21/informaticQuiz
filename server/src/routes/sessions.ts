...
sessionsRouter.post('/', (req, res, next) => {
  try {
    ...
    if (
      ...
      !['topic', 'challenge', 'campaign'].includes(mode)
    ) {
    ...
    if (mode === 'topic') {
      questionRows = db
        .prepare('SELECT * FROM questions WHERE topic_id = ? ORDER BY RANDOM() LIMIT 20')
        .all(topicId);
    } else if (mode === 'challenge') {
      const limit = Math.min(count || 10, 20);
      questionRows = db
        .prepare(
          'SELECT questions.* FROM questions JOIN topics ON questions.topic_id = topics.id WHERE topics.grade = ? ORDER BY RANDOM() LIMIT ?',
        )
        .all(grade, limit);
    } else {
      // campaign
    ...
    const insertTx = db.transaction((rows: any[]) => {
      db.prepare(
        'INSERT INTO quiz_sessions (id, student_name, grade, mode, topic_id, level) VALUES (?,?,?,?,?,?)',
      ).run(sessionId, studentName.trim(), grade, mode, topicId || null, level || null);
...
sessionsRouter.get('/:id/questions', (req, res, next) => {
  try {
    const session = db.prepare('SELECT * FROM quiz_sessions WHERE id = ?').get(req.params.id) as
      QuizSessionRow | undefined;
    if (!session) throw new ApiError(404, 'SESSION_NOT_FOUND');
    const rows = db
      .prepare(
        'SELECT q.* FROM session_questions sq JOIN questions q ON sq.question_id = q.id WHERE sq.session_id = ? ORDER BY sq.order_index',
      )
...
sessionsRouter.post('/:id/submit', (req, res, next) => {
  try {
    ...
    const answers = (Array.isArray(req.body?.answers) ? req.body.answers : []) as {
      questionId: number;
    ...
    const rows = db
      .prepare(
        'SELECT q.* FROM session_questions sq JOIN questions q ON sq.question_id = q.id WHERE sq.session_id = ?',
      )
      .all(req.params.id) as any[];
    rows.forEach((q) =>
    ...
    const insertAnswer = db.prepare(
      'INSERT INTO session_answers (session_id, question_id, answer, is_correct, points_earned) VALUES (?,?,?,?,?)',
    );
    ...
    const finishSession = db.prepare(
      "UPDATE quiz_sessions SET finished_at = datetime('now') WHERE id = ?",
    );
    ...
    const submitTx = db.transaction(() => {
      ...
      const scoreResult = insertScore.run(
        req.params.id,
...
sessionsRouter.get('/:id/result', (req, res, next) => {
  try {
    ...
    const answers = db
      .prepare(
        'SELECT sa.*, q.prompt, q.type FROM session_answers sa JOIN questions q ON sa.question_id = q.id WHERE sa.session_id = ?',
      )
      .all(req.params.id);
