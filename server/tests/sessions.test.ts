...
describe('POST /api/sessions', () => {
  ...
  it('creates a challenge session with question limit', async () => {
    const res = await request(app).post('/api/sessions').send({ studentName: 'Budi', grade: 7, mode: 'challenge', count: 2 });
    expect(res.status).toBe(201);
    expect(res.body.sessionId).toBeTruthy();
  ...
  it('creates a campaign session matching level difficulty', async () => {
    const res = await request(app).post('/api/sessions').send({ studentName: 'Citra', grade: 7, mode: 'campaign', topicId: 1, level: 1 });
    expect(res.status).toBe(201);
    expect(res.body.sessionId).toBeTruthy();
  ...
  it('rejects invalid mode or missing studentName', async () => {
    const res1 = await request(app).post('/api/sessions').send({ studentName: '', grade: 7, mode: 'topic', topicId: 1 });
    expect(res1.status).toBe(400);

    const res2 = await request(app).post('/api/sessions').send({ studentName: 'Andi', grade: 7, mode: 'unknown', topicId: 1 });
    expect(res2.status).toBe(400);
  });

  it('returns 404 when no questions match', async () => {
    const res = await request(app).post('/api/sessions').send({ studentName: 'Andi', grade: 7, mode: 'topic', topicId: 999 });
...
describe('GET /api/sessions/:id/result', () => {
  ...
  it('returns result with score, answers, level, and topicId (pre-flight ruling)', async () => {
    ...
    await request(app).post(`/api/sessions/${created.body.sessionId}/submit`).send({
